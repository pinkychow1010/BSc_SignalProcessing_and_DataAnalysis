...
for i in range(1, len(regions)-1):
    s = n/(sigma**2)
    i = int(i)
    if regions[i][1] == 'Ocean':#Ocean: 0.03
        sigma = 0.03
        C[:,i] *= s          
    elif regions[i][1] == 'Land': #Land: 0.04
        sigma = 0.04
        C[:,i] *= s
    elif regions[i][1] == 'Grnld': #Greenland: 0.3
        sigma = 0.3
        C[:,i] *= s
    elif regions[i][1] == 'Antar': #Antarctica: 0.3
        sigma = 0.3
        C[:,i] *= s 
    elif regions[i][1] == 'Alask': #Alaska: 0.3
        C[:,i] *= 0.3
...
The plot shows less striping in the ocean, meaningful hydrological signals such as mass loss in the\
amazon due to evaporation and mass loss in the third pole. Other signals can be shown clearly in the plot includes\
mass loss in Japan and in North Australia, and mass gain in Southern China.") 
